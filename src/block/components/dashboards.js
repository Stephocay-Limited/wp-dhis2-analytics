        ...
        if (dashboards) {
            listDashboards = dashboards.filter(d=>d.dashboardItems.length > 0).map((dashboard) =>
                <li className="a-items">
                    ...
                    <label for={dashboard.id}>{dashboard.name}</label>
                    <div className="a-content">
                        <ul className="dashboard-items">
                            {

                                dashboard.dashboardItems.map((dashboardItem) => {
                                    var type = dashboardItem.type;
                                    var icon = <Help className="item-type-icon" />;
                                    if (type == "CHART") {
                                    ...
                                    let isChecked = true;
                                    return <li className="dashboard-item">
                                        <label for={dashboardItem.data.id} className="dashboard-item-label">
                                            {icon}
